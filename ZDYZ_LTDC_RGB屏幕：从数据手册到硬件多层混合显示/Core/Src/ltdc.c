...
/* USER CODE BEGIN 0 */
#define LCD_FB0_ADDRESS  0xC0000000                         // 层0起始地址
#define LCD_FB1_ADDRESS  (0xC0000000 + 800 * 480 * 2)      // 层1紧随其后（偏移750KB）
/* USER CODE END 0 */
...
void MX_LTDC_Init(void)
{
  ...
  /* USER CODE BEGIN LTDC_Init 0 */
    ...
    uint16_t *ltdc_fb0 = (uint16_t *)LCD_FB0_ADDRESS;
    uint16_t *ltdc_fb1 = (uint16_t *)LCD_FB1_ADDRESS;

    // 1. 给层 0 刷全屏蓝色
    for (uint32_t i = 0; i < (800 * 480); i++) {
    ltdc_fb0[i] = 0x001F; // RGB565 纯蓝色
    }

    // 2. 给层 1 刷红色 (400x240 大小)
    for (uint32_t i = 0; i < (400 * 240); i++) {
    ltdc_fb1[i] = 0xF800; // RGB565 纯红色
    }
  ...
  pLayerCfg.FBStartAdress = LCD_FB0_ADDRESS;
  ...
  //在这边暂停观看效果！看看混合层的效果，在后续进阶的时候可以尝试一下
 while(1);

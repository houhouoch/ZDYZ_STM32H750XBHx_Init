...
| 设计工具 | 应用场景 | 典型示例 |
| :--- | :--- | :--- |
| **互斥锁 (Mutex)** | **独占硬件资源**。保护具有原子性的物理操作，防止寄存器配置被多任务改乱。 | **DMA2D 寄存器配置**、SPI 总线切换、全局参数结构体读写。 |
| **消息队列 (Queue)** | **逻辑解耦与异步通知**。跨任务传递数据，实现“生产者-消费者”模型。 | **CAN 接收任务** 收到数信号后放入队列，**GUI 任务** 监听队列并刷新屏幕数值。 |

...
/* 1. 检查是否为“传输完成”中断 (Transfer Complete) */
    if (DMA2D->ISR & DMA2D_ISR_TCIF) 
    {
        /* 2. 清除传输完成标志位 (注意使用 IFCR 寄存器) */
        DMA2D->IFCR |= DMA2D_IFCR_CTCIF;
//        /* 3. 唤醒等待绘图的 RTOS 任务 */
//        /* 假设你的 GUI 任务句柄是 GUI_TaskHandle */
//        extern osThreadId_t GUI_TaskHandle; 
//        if (GUI_TaskHandle != NULL)
//        {
//            osThreadFlagsSet(GUI_TaskHandle, 0x01);
//        }
        extern volatile uint8_t g_dma2d_done;
      g_dma2d_done = 1;
       // return;
    }
